...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        int ppValidOperMin = 0;
        int ppValidOperMax = 50;
        // Переменные определяющие рамки условий, можно менять, соответственно изменятся условия - Конец

        //Предворительные проверки и вычисления - Начало
        if (ppValidOperMin > ppValidOperMax) {
            throw new Exception("Error: Здан некоректный диапазон, минимальное значение больше максимального");
        }
        String ppValidnyeSimvoly = " " + ppValidnyeSimvolyArab + ppValidnyeSimvolyRim + String.join("", ppMasOperant);
        //Предворительные проверки и вычисления - Конец

        //Диалог с пользователем (не обязательный код, можно закоментировать) - Начало
        System.out.println("Введите в одну строчку выражения для операций между двумя числами." +
                ...
                "\n   Калькулятор умеет работать полько с Арабскими или Римскими числами одновременно." +
                ...
                "\n   10 - 5" +
                "\n");
        //Диалог с пользователем (не обязательный код, можно закоментировать) - Конец

        System.out.print("Введите вырожение: ");
        Scanner scanner = new Scanner(System.in);
        String ppSring001 = scanner.nextLine();

        //Исправления опечаток (не обязательный код, можно закоментировать) - Начало
        ...
        //Удаление лишних пробелов с начала введенного вырожения
        while (ppSring001.charAt(0) == ' ') {
            ppSring001 = ppSring001.substring(1);
        }
        //Удаление лишних пробелов с конца введенного вырожения
        ...
        //Удаление лишних пробелов между символами в вырожении
        while (ppSring001.contains("  ")) {
            ppSring001 = ppSring001.replace("  ", " ");
        }
        //Исправления опечаток (не обязательный код, можно закоментировать) - Конец

        //Разделение строки
        String[] ppMasElements = ppSring001.split(" ");
        //Остановка если не три элемента
        if (ppMasElements.length != 3) {
            System.out.println("Error: вырожение должно состоять из трех элементов");
            throw new Exception("т.к. строка не является матиматической операцией");
        }
        //Остановка если второй элемент не оператор
        if (!String.join("", ppMasOperant).contains(ppMasElements[1])) {
            System.out.println("Error: второй элемент не оператор из  списка разрешонных: " + Arrays.stream(ppMasOperant).toList());
            throw new Exception("т.к. строка не является матиматической операцией");
        }
        ...
        if (pfTips(ppMasElements[0], ppValidnyeSimvolyArab) && pfTips(ppMasElements[2], ppValidnyeSimvolyArab)) {
            ...
            System.out.println("Цифры арабские, Будет вычислятся: [" + ppSring001 + "]");
            ...
            System.out.println("Цифры Римские, Будет вычислятся: [" + ppSring001 + "]");
    ...
    public static int pfResult(int ppE0, String ppE1, int ppE2, int ppMin, int ppMax) throws Exception {
        ...
        if (ppE0 >= ppMin && ppE0 <= ppMax && ppE2 >= ppMin && ppE2 <= ppMax) {
            ...
            System.out.println("Минимильное  разрешонное число в программе: " + ppMin + "\nМаксимальное разрешонное число в программе: " + ppMax);
            throw new Exception("Error: Как минимум одно из чисел не отвечает требуемуму диапазону");
        }
        return r;
    }
