...
//====================================================================================================================//
    ...
    public static int pfNumArab(String ppElement, String[][] ppMMRimX) throws Exception {
        ...
        String[] ppMSubRimX;
        int ppMaxSimv;
        for (int ppRazryad = 1000; ppRazryad > 0; ppRazryad = ppRazryad / 10) {       //Четыре итерации поиска: Тысячи, Сотни, Десятки, Единицы.
            ppMSubRimX = ppMMRimX[((String.valueOf(ppRazryad).length() - 4) * (-1))]; //((String.valueOf(ppRazryad).length() - 4) * (-1)) выдает:  0, 1, 2, 3.
            ...
            for (int ppSimvKolVo = ppMaxSimv; ppSimvKolVo > 0; ppSimvKolVo--) {       //Если пришло [XXVII], 1 цикл проверка соответствия [XXVI], 2 цикл проверка соответствия [XXV], 3 цикл проверка соответствия [XX].
                for (int i = 0; i < ppMSubRimX.length; i++) {                         //В данном примере производится проверка соответствия с этим: X, XX, XXX, XL, L, LX, LXX, LXXX, XC. (Десятки)
                    if (ppMSubRimX[i].equals(ppNumRim.substring(0, ppSimvKolVo))) {   //В итоге найдется соответствие с [XX].
                        ppNumArab = ppNumArab + (ppRazryad * i);                      //Если пришло [XXVII], найдется [XX] зафиксируется как 2, Далее A=A+(2*i), где i разрядность, в этом примере i=10. (Десятки)
